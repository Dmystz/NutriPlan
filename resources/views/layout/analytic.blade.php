...
@section('content')
                    ...
                    <div class="row g-2 mb-3">
                        ...
                        <div class="col-6">
                            ...
                            <select id="activity" class="analytic-input">
                                <option value="1.2"   {{ ($user->activity_level ?? 1.55) == 1.2   ? 'selected' : '' }}>Sedentary</option>
                                <option value="1.375" {{ ($user->activity_level ?? 1.55) == 1.375 ? 'selected' : '' }}>Lightly active</option>
                                <option value="1.55"  {{ ($user->activity_level ?? 1.55) == 1.55  ? 'selected' : '' }}>Moderately active</option>
                                <option value="1.725" {{ ($user->activity_level ?? 1.55) == 1.725 ? 'selected' : '' }}>Very active</option>
                                <option value="1.9"   {{ ($user->activity_level ?? 1.55) == 1.9   ? 'selected' : '' }}>Extra active</option>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-6">
                            ...
                            <div class="input-unit-wrap">
                                <input type="number" id="height-input" class="analytic-input pe-5"
                                    value="{{ $user->tinggi_badan ?? 165 }}" min="50" max="250" oninput="calcBMI()">
                        ...
                        <div class="col-6">
                            ...
                            <div class="input-unit-wrap">
                                <input type="number" id="weight-input" class="analytic-input pe-5"
                                    value="{{ $user->berat_badan ?? 58 }}" min="1" max="300" step="0.1" oninput="calcBMI()">
                        ...
                        <div class="analytic-card h-100 text-center">
                            ...
                            <div class="gauge-wrap">
                                <svg viewBox="0 0 200 120" class="gauge-svg">
                                    <path d="M20,100 A80,80 0 0,1 60,26"  stroke="#60A5FA" stroke-width="16" fill="none" stroke-linecap="round"/>
                                    <path d="M62,25  A80,80 0 0,1 110,20" stroke="#34D399" stroke-width="16" fill="none" stroke-linecap="round"/>
                                    <path d="M112,20 A80,80 0 0,1 155,38" stroke="#FBBF24" stroke-width="16" fill="none" stroke-linecap="round"/>
                                    <path d="M157,40 A80,80 0 0,1 180,100" stroke="#F87171" stroke-width="16" fill="none" stroke-linecap="round"/>
                            ...
                            <p class="fw-bold mt-2 mb-1" id="bmi-label" style="font-size:1rem;">● {{ $bmiStatus ?? 'Normal' }}</p>
                    ...
                    <div class="col-12 col-sm-6">
                        <div class="analytic-card h-100">
                            ...
                            @php
                                $bv  = $bmi ?? 0;
                                $cls = [
                                    ['id'=>'cls-underweight','color'=>'#60A5FA','label'=>'Underweight', 'range'=>'< 18.5',      'active'=>$bv < 18.5],
                                    ['id'=>'cls-normal',     'color'=>'#34D399','label'=>'Normal weight','range'=>'18.5 – 24.9','active'=>$bv >= 18.5 && $bv < 25],
                                    ['id'=>'cls-overweight', 'color'=>'#FBBF24','label'=>'Overweight',  'range'=>'25.0 – 29.9', 'active'=>$bv >= 25   && $bv < 30],
                                    ['id'=>'cls-ob1',        'color'=>'#fb923c','label'=>'Obesity I',   'range'=>'30.0 – 34.9', 'active'=>$bv >= 30   && $bv < 35],
                                    ['id'=>'cls-ob2',        'color'=>'#f97316','label'=>'Obesity II',  'range'=>'35.0 – 39.9', 'active'=>$bv >= 35   && $bv < 40],
                                    ['id'=>'cls-ob3',        'color'=>'#ef4444','label'=>'Obesity III', 'range'=>'≥ 40.0',      'active'=>$bv >= 40],
                                ];
                            @endphp
                            <div class="bmi-class-list">
                                @foreach($cls as $c)
                                <div class="bmi-class-row {{ $c['active'] ? 'active-class' : '' }}" id="{{ $c['id'] }}">
                                    <span class="bmi-dot" style="background:{{ $c['color'] }};"></span>
                                    <span class="bmi-class-name {{ $c['active'] ? 'fw-bold' : '' }}">{{ $c['label'] }}</span>
                                    <span class="bmi-you-badge ms-1" style="display:{{ $c['active'] ? 'inline' : 'none' }};">YOU</span>
                ...
                <div class="analytic-card h-100">
                    ...
                    <div class="summary-list">
                        ...
                        <div class="summary-row">
                            ...
                            <span class="summary-value fw-bold" style="color:#ea5c2b;" id="sum-bmi">{{ number_format($bmi ?? 0, 1) }}</span>
                        </div>
                        <div class="summary-row">
                            <span class="summary-icon">🎯</span>
                ...
                <div class="analytic-card h-100">
                    <div class="analytic-card-header mb-1">
                        ...
                        <select class="ms-auto analytic-input"
                            style="max-width:90px; font-size:0.72rem; padding:0.15rem 0.5rem;"
                            id="history-year">
                ...
                <div class="cta-banner d-flex align-items-center justify-content-between flex-wrap gap-3">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="mb-0 text-muted" style="font-size:0.83rem;">Get a personalized meal plan that fits your body and goals!</p>
                        </div>
                    </div>
    ...
    <style>
        ...
        .analytic-card-header { display:flex; align-items:center; gap:0.5rem; font-size:0.9rem; margin-bottom:0.75rem; }
        .analytic-icon { font-size:1.1rem; }
        ...
        .gender-card.active-female .gender-symbol { color: #ea5c2b; }
        .gender-card.active-male   .gender-symbol { color: #3B82F6; }
        .gender-card-label {
            font-size: 0.8rem;
            font-weight: 600;
            color: #6B7280;
        ...
        .gender-card.active-female .gender-card-label { color: #ea5c2b; }
        .gender-card.active-male   .gender-card-label { color: #3B82F6; }
        .gender-check {
            width: 16px;
            height: 16px;
            border-radius: 50%;
        ...
        .gender-card.active-female .gender-check { background: #ea5c2b; border-color: #ea5c2b; }
        .gender-card.active-male   .gender-check { background: #3B82F6; border-color: #3B82F6; }

        /* ── Form Inputs ─────────────────────────────────────────────── */
        .analytic-label { font-size:0.78rem; color:#374151; font-weight:600; display:block; margin-bottom:0.2rem; }
        .analytic-input { width:100%; border-radius:10px; border:1.5px solid #E5E7EB; background:#F9FAFB; padding:0.4rem 0.75rem; font-size:0.85rem; color:#374151; outline:none; transition:border-color 0.2s; }
        .analytic-input:focus { border-color:#95cd41; }
        .input-unit-wrap { position:relative; }
        .unit-badge { position:absolute; right:0.65rem; top:50%; transform:translateY(-50%); font-size:0.72rem; color:#9CA3AF; pointer-events:none; }

        /* ── Calculate Button ────────────────────────────────────────── */
        .btn-calculate { display:flex; align-items:center; justify-content:center; gap:0.5rem; border:none; border-radius:50px; background:linear-gradient(90deg,#95cd41 0%,#ea5c2b 100%); color:#fff; font-weight:700; font-size:0.95rem; padding:0.65rem 1rem; cursor:pointer; box-shadow:0 4px 14px rgba(149,205,65,0.35); transition:all 0.25s; }
        ...
        .gauge-value-wrap { position:absolute; bottom:8%; left:50%; transform:translateX(-50%); text-align:center; line-height:1.1; }
        .gauge-number { font-size:1.8rem; font-weight:800; color:#1F2937; }
        .gauge-unit { font-size:0.65rem; color:#6B7280; display:block; }

        /* ── BMI Classification ──────────────────────────────────────── */
        .bmi-class-list { display:flex; flex-direction:column; gap:0.35rem; }
        .bmi-class-row { display:flex; align-items:center; gap:0.5rem; border-radius:10px; padding:0.35rem 0.6rem; font-size:0.83rem; transition:background 0.2s; }
        .bmi-class-row.active-class { background:rgba(149,205,65,0.12); }
        .bmi-dot { width:10px; height:10px; border-radius:50%; flex-shrink:0; }
        .bmi-class-name { font-size:0.83rem; color:#374151; }
        .bmi-class-range { font-size:0.78rem; color:#6B7280; }
        .bmi-you-badge { font-size:0.6rem; font-weight:700; background:#95cd41; color:#fff; border-radius:50px; padding:1px 6px; }

        /* ── Summary ─────────────────────────────────────────────────── */
        .summary-list { display:flex; flex-direction:column; gap:0; }
        .summary-row { display:flex; align-items:center; gap:0.5rem; padding:0.45rem 0; border-bottom:0.8px solid rgba(0,0,0,0.06); font-size:0.83rem; }
        .summary-row:last-child { border-bottom:none; }
        .summary-icon { font-size:1rem; }
        .summary-label { color:#6B7280; }
        .summary-value { margin-left:auto; font-weight:500; color:#1F2937; }
        .badge-category { margin-left:auto; font-size:0.72rem; font-weight:700; background:rgba(149,205,65,0.15); color:#446611; border-radius:50px; padding:2px 10px; }

        /* ── Insight Cards ───────────────────────────────────────────── */
        .insight-card { display:flex; align-items:flex-start; gap:0.6rem; border-radius:12px; padding:0.65rem 0.75rem; }
        .insight-green  { background:rgba(220,252,231,0.6); }
        .insight-orange { background:rgba(255,237,213,0.6); }
        .insight-blue   { background:rgba(219,234,254,0.6); }
        .insight-red    { background:rgba(254,226,226,0.6); }
        .insight-icon-wrap { font-size:1.1rem; border-radius:50%; width:32px; height:32px; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
        .insight-title { font-size:0.82rem; font-weight:700; margin-bottom:2px; }
        .insight-body  { font-size:0.75rem; color:#374151; margin-bottom:0; }
        ...
        @media (max-width:767px) {
            .analytic-card { padding:1rem; }
            .cta-banner { padding:1rem 1.25rem; }
            .gauge-number { font-size:1.4rem; }
        }
        @media (max-width:480px) {
            .bmi-class-row { font-size:0.75rem; }
    ...
    <script>
        ...
        function calcBMI() {
            ...
            if (bmi < 18.5) {
                label = '● Underweight'; color = '#60A5FA'; catText = 'Underweight';
                msg = isMale
                ...
                label = '● Normal Weight'; color = '#34D399'; catText = 'Normal Weight';
                msg = isMale
                    ? 'Great! Your BMI is normal. Estimated body fat: ' + bodyFatDisplay + '% — healthy range for men is 8–20%.'
                    : 'Great! Your BMI is normal. Estimated body fat: ' + bodyFatDisplay + '% — healthy range for women is 21–33%.';
                needleAngle = mapRange(bmi, 18.5, 25, -30, 10);
            } else if (bmi < 30) {
                label = '● Overweight'; color = '#FBBF24'; catText = 'Overweight';
                msg = isMale
                    ? 'BMI above normal. Estimated body fat: ' + bodyFatDisplay + '% — men above 25% body fat carry higher metabolic risk.'
                    : 'BMI above normal. Estimated body fat: ' + bodyFatDisplay + '% — women above 33% body fat carry higher metabolic risk.';
                needleAngle = mapRange(bmi, 25, 30, 10, 50);
            } else if (bmi < 35) {
                label = '● Obesity I'; color = '#fb923c'; catText = 'Obesity I';
                msg = 'Estimated body fat: ' + bodyFatDisplay + '%. Please consult a healthcare professional for a personalized plan.';
                needleAngle = mapRange(bmi, 30, 35, 50, 70);
            } else if (bmi < 40) {
                label = '● Obesity II'; color = '#f97316'; catText = 'Obesity II';
                msg = 'Your BMI indicates Obesity II. Medical guidance is strongly recommended.';
                needleAngle = mapRange(bmi, 35, 40, 70, 85);
            } else {
                label = '● Obesity III'; color = '#ef4444'; catText = 'Obesity III';
            ...
            const active  = [bmi<18.5, bmi>=18.5&&bmi<25, bmi>=25&&bmi<30, bmi>=30&&bmi<35, bmi>=35&&bmi<40, bmi>=40];
        ...
        function updateInsights(bmi, gender, bodyFatPct) {
            ...
            if (bmi < 18.5) {
                insights = [
                    { cls:'insight-blue',  bg:'#DBEAFE', icon:'🍽️', color:'#2563EB',
                    title: isMale ? 'Increase protein & calories' : 'Focus on nutrient-dense foods',
                    body:  isMale ? 'Prioritize lean protein (chicken, eggs, legumes) and complex carbs to build mass.'
                                    : 'Include healthy fats, whole grains, and iron-rich foods to reach a healthy weight.' },
                    { cls:'insight-orange',bg:'#FFEDD5', icon:'🏋️', color:'#ea5c2b',
                    ...
                    { cls:'insight-green', bg:'#DCFCE7', icon:'💧', color:'#16A34A',
                    title:'Stay hydrated!',
                ...
                insights = [
                    ...
                    { cls:'insight-orange',bg:'#FFEDD5', icon:'🏃', color:'#ea5c2b',
                    ...
                    { cls:'insight-blue',  bg:'#DBEAFE', icon:'💧', color:'#2563EB',
                    title:'Stay hydrated!',
                ...
                insights = [
                    { cls:'insight-red',   bg:'#FEE2E2', icon:'🏥', color:'#DC2626',
        ...
        async function calcAndSaveBMI() {
            ...
            try {
                ...
                btn.disabled  = false;
                btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg> Calculate &amp; Save BMI';
        ...
        function updateHistoryInsight(data) {
            ...
            else if (diff < 0)         el.textContent = 'Great progress! Your BMI dropped by ' + Math.abs(diff) + ' this year. Keep it up!';
            else                       el.textContent = 'Your BMI increased by ' + diff + ' this year. Consider adjusting your diet and activity level.';
        }
            ...
            bmiChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: { ticks: { font: { size: 9 }, stepSize: 1 }, grid: { color: 'rgba(0,0,0,0.05)' }, min: 15, max: 40 }
                    }
                }
            });
